...
const CartItem: React.FC<ICartItem> = ({
	...
	return (
		<div className={classes.CartItemContent}>
			<img className={classes.CartItemImg} src={productImg} alt={productId} />
			<div className={classes.CartItemName}>
				<Link to={`/products/${productId}`} title={productTitle}>
					{productTitle}
				</Link>
				<div className={classes.CartItemPrice}>
					$ {productPrice} x {amount} :
					<br />
					<span>$ {(productPrice * amount ?? 0).toFixed(2)}</span>
				</div>
			</div>
			<div className={classes.CartItemBtns}>
				<AmountBtns amount={amount} increase={increase} decrease={decrease} />
				<CustomBtn onClick={() => removeItem(productId)}>Удалить</CustomBtn>
			</div>
		</div>
	);
};
